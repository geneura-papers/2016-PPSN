...
A BYOD policy has been integrated in several environments, mainly in % I'm changing some to policy, which is what it is - JJ
...
Nevertheless and whatever the BYOD environment is, the biggest worry and the main issue is all about privacy and security \cite{miller2012byod}.% if the issues are the same, why mention different environments separately? Besides, what assets of the learning center are you actually accessing? - JJ
...
technique is proposed. % The first claim about this paper can't be this imprecise. Say exactly what you want to do, not "going behond". Say that you will be helping define and refine those rules automatically, instead of having the CSO imagining and implementing, by herself, all the needed rules - JJ
...
rules. % in order to have the space of possible policy rules be as wide as possible, we will need a techinque that explores the rule space efficiently and with as little structure as possible, that is why we introduce...
...
respect privacy while trying to secure the resources. % this is a
