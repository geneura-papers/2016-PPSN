...
As previously highlighted, in this work we propose a system which is able to process a set of user actions that have been allowed or denied based on initial, simple rules, and discover new one by exploring the whole space of possible combinations among the attribute values. The proposed method uses GP to create a tree to model the different rules, given that the problem can be seen as a classification one and therefore the model can be a decision tree \cite{safavian1990survey}. The generated tree
