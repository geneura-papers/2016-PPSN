...
of classification rules, obtaining good fitness, and presenting the rules in a way that is helpful for a CSO. We demonstrate so by using the developed framework with a real-world dataset, and comparing two ways of coding the individuals: as a set of rules, or as a single rule. %Pablo: complete this paragraph with ", to establish blabla"
% (Paloma) Establish... what? I think I don't get your comment :(
...
rules for the PROBEN1 databases via GP is described and a new fitness is introduced. As happened in %Pablo: describe what the PROBEN databases are.
the survey of Espejo et al., from the six databases inside PROBEN1 and
analysed by these authors, none is related to security. However, the authors' proposed fitness is indeed of interest for this research, and as such we compare the performance or our algorithm using two different fitness. Section \ref{subsec:solution} will describe in detail which solution has been built from what has been found in the work of Freitas. %Pablo: it is not a good practice to refer to future sections, maybe a small paragraph justifying the use of the fitness here would be better 
...
maximum patterns. The last step would be to present the rules to the %Pablo: explain here we are going to use 2 individuals and 2 fitness
