...
access by establishing a policy based on classification rules with the less false positives as possible. Our
...
Through these comparisons, we can establish a proof of concept to demonstrate the viability, effectiveness, and applicability of the GP approach, in certain conditions, to the BYOD security context. 
\end{abstract}
...
event or user action as permitted or not permitted. The objective, thus, is to obtain a way of correctly classifying the most incoming events as possible, avoiding mostly the false positives, which means that events that should be non permitted must never be. This could be used
...
Once the variability of the obtained fitness has been studied, and always taking into account that those values come from their evaluation in a training subset of the data, now we \textit{validate} the proposed approaches with a test set. The way to evaluate this is similar to Equation \ref{eq:accuracy}, but using the test subset of the data:
...
This measure will be the same no matter what approach has been taken nor which fitness function has been used to evaluate the individuals. Table \ref{tab:PvsMvalidation} shows the average, best, median, and worst results from the evaluations with $f_{Acc}$ and also $f_{CS}$.
...
As it can be also seen in Table \ref{tab:PvsMvalidation}, results obtained for class STRONGDENY in the Michigan approach are very poor comparing with the GRANTED class. This means that the highly imbalanced data set that we are using clearly biases the results towards the majority class \cite{japkowicz2002class}, and in this case there are 1 instance in the training set of data labelled as STRONGDENY for every 13 labelled as GRANTED. In this sense, solutions can be found in literature in order to reduce the bias while performing classification tasks, such as in \cite{chawla2005data} and \cite{sun2009classification}. Also, in \cite{bhowan2012developing},
the authors present a comparison between many different types of fitness functions, testing on various unbalanced datasets, with different minority-class/majority-class ratios.

To evaluate computation costs of the two approaches, and taking into consideration the infrastructure available for the experiments (see Section\ref{sec:experiments}), the execution times are presented in Table \ref{tab:PvsMtime}, detailed by their average value and standard deviation (sd), along weith the best, worst, and median values. Values are expressed in hours. Values for the Michigan approach are not separated by class this time, because in order to have the two rules - one for each class -- the CSO would have to wait for both executions.
...
\begin{table*}
	...
	\end{tabular}
	\caption{Comparison between Pittsburgh and Michiganapproaches, in terms of their execution time. For the Michigan approach, times for executing the algorithm once for the GRANTED class and another for the STRONGDENY are presented together. The best value is not indicated given that we did not find statistically significant differences.}
	\label{tab:PvsMtime}
\end{table*}
