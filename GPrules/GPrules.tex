...
% Delete comment above when accepted or rejected - JJ
...
Rules are constructed starting from each leaf of the tree and iterating to the upper parents, or variables, reading their data as string. Therefore, the number of rules of the set produced by the tree is equal to the number of its leafs. It is worth mentioning that some variables may contradict others during the evolution. This is not a problem because those rules will not cover any instance and thus they will not contribute to the fitness value. %PABLO: changed "rules" to "variables", please Paloma, confirm that it is ok
...
a list of conditions, with a fixed class, obtaining just one rule for
...
how it interacts with what the others cover \cite{freitas2002data}, it
can lead to massive overlapping with the consequent loss of
efficiency. 

% This could go above, where you talk about variables - JJ
% (Paloma) I don't agree, because both approaches have this in common. If we move this before explaining the secons approach, it may break the reading. Let's wait for Pablo's opinion Pablo: maybe we can add to this phrase: "With respect to the variables, both approaches use..." to remark that the variables are the same, well... in both approaches :P
Furthermore, the variables can be differentiated into three types, 
...
the most used metric to calculate it is the accuracy %Pablo: the most used "it" is? With "it"? (serious question, IDK not much about some grammatical expressions :P)
...
of $f_{Acc}$ is the whole training dataset, $T_{tr}$, and  has the %Pablo: it was written f_c instead f_acc
...
16 parents have been used. These parameters have been also previously used in previous GP works \cite{EvoStar2014:GPBot}. Table \ref{tab:parameters} summarizes all the parameters used.
...
As our purpose in this paper is to obtain a system which proposes to a CSO useful security rules that improve the existing ones, and present them in an understandable way and as fast as possible, %Pablo: are we playing with fire with this sentence? Some reviewer may say "there are faster methods than GP". We can remove it, or use it mentioning the time difference from both approaches
in this section we compare the results from using two fitness functions
...
the proposed approaches with a validation set, similar to the test set used in classification problems \cite{witten2005data}. 
...
and now it is more clear, given that the accuracy over the test set is
not even 50\%. This difference means that the highly imbalanced data
set that we are using clearly biases the results towards the majority
class \cite{japkowicz2002class}, and in this case there are 1 instance
in the training set of data labelled as STRONGDENY for every 13
labelled as GRANTED. In this sense, solutions can be found in
literature in order to reduce the bias while performing classification
tasks, such as in \cite{chawla2005data} and
...
As we have executed the experiments performing a 10-fold cross-validation, in order to study the best individuals for each configuration we need to consider the values of the validation accuracies per fold, and then choose the best one. In Figure \ref{fig:bestInd_fCS_a0} we show an example of best individual, in this case obtained when using $f_{CS}$ as the fitness function with an $\alpha$ value of 0. The tree in this figure represents a set of 16 rules, two of them classifying to the STRONGDENY class, whilst the rest classify towards the GRANTED class. The ones that classify to STRONGDENY have been highlighted and presented in string form in the figure. %Pablo: maybe to add something interesting about the tree. For example, the STRONGDENY rules imply that the consultancy users are very important, and the not simple events may be dangerous
...
The STRONGDENY rules imply that the developers are more probably to get a GRANTED in their actions, and this conclusion makes sense because because they are more familiar with system security. Also, from the second rule we can infer that the complex events -- not simple -- and the devices owned by the users and not the company may be dangerous.
...
obtained individuals. A typical way of approaching this problem is to
use balancing techniques, such as ``oversampling'' \cite{smote_02}, in
...
2015 UGR). %Is this one still working? - JJ Pablo: and also, should we add MUSES, even if it was finished? (in fact, the results were generated there and part of the paper was written during the contract :P)
