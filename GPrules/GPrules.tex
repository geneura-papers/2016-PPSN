...
policies and wants to simplify or generalize them; a second scenario
...
% (Paloma) Done!
% You have not mentioned you are comparing different fitness
% functions. You _really_ have to characterize the problem you are
% tackling in the introduction. If you are comparing fitness, you have
% to say that the problem in this context is to find a fitness
% function. Representation is not problem specific, so you have to
% proceed from the generic (algorithm domain) to the specific (problem
% domain) when explaining what you are going to do in this particular
% paper. Are conclusions about codification specific to the problem
% domain? - JJ
...
% You should say somewhere whay are they specific. You can have WiFi
% encryption with a company-provided device, for
% instance. DeviceOwnedBy should be BYOD specific, for instance. if
% it's not owned by the user, the context is _still_ BYOD. - JJ
\label{tab:variables}
\end{center}
\end{table*}
...
very harmful. % Which ones are harmful? - JJ
...
C_{ind} = TP + TN - (FP + FN)
\end{equation}
% You have not defined FP and FN - JJ
...
to check how they influence in the solution % They influence what in
                                % the solution? - JJ
...
\caption{Best fitness obtained when the individual is coded as a set
  ...
  for $\alpha$. %In that case, it should be by the side of $\alpha$
}
...
\caption{Best fitness obtained when the individual is coded as a
  ...
  for $F_{CS}$, lower is better.} % no mark for teh best alpha? - JJ
\label{tab:michigan}
\end{center}
\end{table*}

No significant differences have been found between results for the
different values of $\alpha$ % differences between statistical
                             % variables, not parameters like alpha. - JJ
for GRANTED and STRONGDENY, after applying a Friedman test for
GRANTED, and an ANOVA test for STRONGDENY. %this means you have
                                %different alphas for granted and
                                %strongdeny? please clarify - JJ
ANOVA has been used in the STRONGDENY results instead Friedman because these distributions follows a normal curve.
