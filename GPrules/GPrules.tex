...
\begin{abstract}
The constant enhancement of personal devices has inspired the adoption of the ?Bring Your Own Device? (BYOD) concept in the corporate world, granting the employees to continue working even if not at the company premises or on virtual private networks. Despite the significant advantages of this policy such as increasing work productivity,  the access to the internal network and the assets it holds by these devices exposes the companies to security threats such as leak of confidential data and unauthorised access. As corporate control is forcefully limited due to privacy, we propose a way of detecting and controlling threats by establishing a policy based on classification rules that allow or deny an action, and mainly focused on having the less false positives as possible. Our approach addresses the problem of automatically obtaining rules to improve BYOD security uses Genetic Programming (GP), since GP is capable of discovering novel and interesting classification rules, with the additional feature of presenting them in an easily understandable way.  We obtain good dataset coverage in the simulation results over real data, after testing different fitness functions and configurations in the way of coding the individuals. Through these comparisons, we can establish a proof of concept to demonstrate the viability, effectiveness, and applicability of the GP approach, in certain conditions, to the BYOD security context for the automatic discovery of rules. 
\end{abstract}
