...
Indeed, each approach has its advantages and disadvantages. To
directly obtain a set of rules (Pittsburgh) able to classify instances
of every existing class means that the algorithm has to be run once,
while having the solution coded as a single rule (Michigan), we have
to obtain as many rules as classes are defined, and so it yields to
many executions of the algorithm. % yields to? - JJ
...
their advantages and disadvantages. %Pablo: between "presented," and "along" I would add something like "to understand how our approach works/our individuals behave" or something like that.
...
In Section \ref{subsec:fitnesscomparison} we shown that the performance in the
...
dangerous action is never taken as permitted. To this end, we have
studied different ways to implement a methodology based on GP, by two
...
The results make a fine proof of concept, %of what concept? %Pablo: a proof-of-concept in general. Maybe write "a fine proof of concept of a system that does..."
as we obtained readable
rules %shown in section XXXX - JJ
and promising rates for the number of false positives (around
7\%). However, since the ideal value for both false positives and
negatives is 0, there is room for improvement. % These conclusions
                                % should be extended. Time needed?
...
improving our system. %this is far too little. Possible improvements
