...
leakage, improper decommissioning, phishing that is focused on a specifig group or company\footnote{Also known as \textit{spear phishing}}, 
...
allowing the system to go beyond the limited set of known pre-defined rules. Additionally, this feature can be used as ``reverse engineering'', so that the rules initially made by the CSO are found in the solution along with additional ones. In order to have the space of possible policy rules be as wide
...
of classification rules, obtaining good fitness, and presenting the rules in a way that is helpful for a CSO. We demonstrate so by using the developed framework with a real-world dataset, and comparing two ways of coding the individuals -- as a set of rules, or as a single rule -- so that we finally propose the approach with best performance. %Pablo: complete this paragraph with ", to establish blabla"
% (Paloma) Done!

The rest of the paper is organised as follows. In Section \ref{sec:SotA} we give an overview of the advances in GP applied to rule evolution and its applications; then, Section \ref{sec:methodology} described the proposed methodology, depicting the problem this work tries to solve and describing the available dataset and the proposed GP framework. The experimental set-up, as well as the different set of experiments that have been carried out are also described in that section. Section \ref{sec:gp} shows the obtained results from the application of GP to security rules extraction and, finally, the conclusions of this work along with some suggestions about how to continue our research are given in Section \ref{sec:future}.  
...
work but anonymised, in the sense that from all the attributes extracted from the user actions, the username is not included as a variable to build rules with. % User data is not specific. What kind of data? - JJ
% (Paloma) Tried to extend it
 ...
 denied, where a warning appears explaining the rule that the user did not comply with and how to perform the action in a more secure way or environment.
								% Are you going to validate those
                                % warnings? To use them in the
                                % training set?
                                % (Paloma) This I didn't get
                                % What is BYOD specific?
                                % about them? - JJ
                                % (Paloma) Tried to explain this
...
\end{itemize}
% A more interesting way of classifying them would be the fact that
% they are BYOD specific or not. All of the examples that you have
% mentioned do not seem to be BYOD specific. - JJ
% (Paloma) Agree, will do

The trials had a duration of a month plus a week, and a total of %Pablo: a month plus a week, or 5 weeks?
...
\textit{log out}, or \textit{restarting the server}. % alternative
                                % rephrasing: We filtered those events
                                % that didn't imply access to assets,
                                % such as... JJ
...
In the case of an individual coded as a set of rules, the tree is translated to a set of rules starting from the leafs to the root node passing each time by the ancestors. However, an individual represented by a single rule, is coded as a list of conditions to which a class is assigned. Thus, an experiment following this last approach has to be run many times as classes there are.
...
% description is the set of steps needed to reproduce them - JJ %PABLO: re-writing and organizing stuff
...
Stop criterion & 150 generations \\ \hline
...
During the fitness evaluation, the generated individual is converted into a string, which can become a single rule or set of rules depending on the approach. Then, the chosen fitness is evaluated for a particular rule - the single rule or each one inside the set - and over the 90\% of the data.
...
in \url{https://github.com/fergunet/GPRuleRefinement}, as a module for 
...
The best obtained fitness when coding the individuals as sets of rules are detailed in Table \ref{tab:pittsburgh}. Values are represented by their mean and standard deviation, given that the algorithm has been executed 10 times, the data folded in a different way each.
...
\multicolumn{2}{|c|}{$F_{Acc}$}                                  & 0.923   & 0.926      & 0.925  & 0.924       \\ \hline
\multicolumn{1}{|c|}{\multirow{3}{*}{$F_{CS}$}} & $\alpha$ = 0   & 7102.2 * & 6198        & 7193   & 7253        \\ \cline{2-5} 
\multicolumn{1}{|c|}{}                          & $\alpha$ = 0.5 & 7235.95 & 7149       & 7250   & 7274        \\ \cline{2-5} 
\multicolumn{1}{|c|}{}                          & $\alpha$ = 1   & 7192.8  & 6620       & 7253.5 & 7298        \\ \hline
\end{tabular}
\caption{Best fitness obtained when the individual is coded as a set of rules, following the Pittsburgh approach, and with 10-fold cross-validation. Two different fitness have been used, and for $\alpha$ values of 0, 0.5, and 1. Note that in the case of $F_{Acc}$ the best value is the maximum, whilst for $F_{CS}$, the best possible value is the minimum. An * indicates the  statistically significant best value for $\alpha$.}
\label{tab:pittsburgh}
\end{center}
\end{table}


In the case of $F_{Acc}$ for the Pittsburgh approach, as the distribution are not normal, a non-parametric test (Friedman) has been used to asset the statistical significance \cite{DerracTests11}, obtaining a p-value of 0.0001. $\alpha$ = 0 is significantly different with $\alpha$ = 0.5 and $\alpha$ = 1, while there are not difference between the two former (p-value = 0.08).
...
\cline{3-6}
                                                &                                 & Class      & Best       & Median   & Worst       \\ \hline
\multicolumn{2}{|c|}{\multirow{2}{*}{$F_{Acc}$}}                                  & GRANTED    & 0.471      & 0.466    & 0.055       \\ \cline{3-6} 
\multicolumn{2}{|c|}{}                                                            & STRONGDENY & 0.463      & 0.048    & 0.043       \\ \hline
\multicolumn{1}{|c|}{\multirow{6}{*}{$F_{CS}$}} & \multirow{2}{*}{$\alpha$ = 0}   & GRANTED    & 25559      & 26274    & 27336       \\ \cline{3-6} 
\multicolumn{1}{|c|}{}                          &                                 & STRONGDENY & 46212      & 47120.5  & 47925       \\ \cline{2-6} 
\multicolumn{1}{|c|}{}                          & \multirow{2}{*}{$\alpha$ = 0.5} & GRANTED    & 26222      & 26757.5  & 27343       \\ \cline{3-6} 
\multicolumn{1}{|c|}{}                          &                                 & STRONGDENY & 46186      & 46624.5  & 47937       \\ \cline{2-6} 
\multicolumn{1}{|c|}{}                          & \multirow{2}{*}{$\alpha$ = 1}   & GRANTED    & 26085      & 26585    & 27333       \\ \cline{3-6} 
\multicolumn{1}{|c|}{}                          &                                 & STRONGDENY & 46210      & 46811    & 47926       \\ \hline
\end{tabular}
\caption{Best fitness obtained when the individual is coded as a single rule, following the Michigan approach, and represented by class. Data was divided in 10-folds to perform cross-validation. Two different fitness have been used, and for $\alpha$ values of 0, 0.5, and 1. Note that in the case of $F_{Acc}$ the best value is the maximum, whilst for $F_{CS}$, the best possible value is the minimum.}
\label{tab:michigan}
\end{center}
\end{table*}

No significant differences have been found between the different values of $\alpha$ for GRANTED and STRONGDENY, after applying a Friedman test for GRANTED, and an ANOVA test for STRONGDENY. ANOVA has been used in the STRONGDENY results instead Friedman because these distributions follows a normal curve.

\subsection{Proposed system configuration}

% Fitness charts are not results. Plus representing 10 different
% time-evolving quantities is _very_ confusing. I cannot see a valid
% reason to publish a single one of these charts, not to mention all
% of them. If what you want to show is differences betwen using one
% fitness or other, just publish the final result, the effort needed
% or anything that proves that you are extracting rules from the
% original policies, several pages of charts with the evolution of
% fitness, which by themselves prove nothing.
